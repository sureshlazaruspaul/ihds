...
gen pop_sq_km = pop / area_sq_km if !missing( pop ) & !missing( area_sq_km )

egen nmiss = rowmiss( nlines lengthlines pop area_sq_km pop_sq_km )
	drop if nmiss > 0
	drop nmiss 
...
label variable pop_sq_km          "Population density (district-wise)" 
