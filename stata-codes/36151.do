...
//------------------------------------------------------------------------------
// Census / Transmission line data ... 
//------------------------------------------------------------------------------

use "https://github.com/sureshlazaruspaul/ihds/blob/main/stata-codes/census_elec.dta?raw=true" , clear

// check duplicates: must be zero
...
keep if inlist( stateid , 0 ) & inlist( distid , 0 )
	drop stateid distid

rename * (=_india)
	gen india = 1 

label variable nlines_india             "Number of transmission lines (india)" 
label variable lengthlines_india        "Total transmission line length (india)" 
label variable area_sq_km_india         "Area (sq-km, india)" 
label variable population_india         "Population (india)" 
label variable population_sq_km_india   "Population density (india)" 

save india_census , replace 
...
merge m:1 india using india_census
	drop if _merge == 2 // left join census and electricity data
	drop _merge india 

capture confirm file india_census.dta

** .. if it does, delete program
if _rc == 0 {
    di "File exists, deleting ..."
    erase india_census.dta
}
