...
// open logfile ... 
//------------------------------------------------------------------------------
log using "C:\ihds\step4_probit_12a", replace
...
foreach m of local depvars { 
	...
	pe keep if inlist( sample, 1 ) // reg sample 
        ...
        pe probit `m' `elecvar' `cvars' `ovars'  , vce( robust ) 
...
esttab using step4_probit_12a.tex , b(a6) se(6) wide starlevels("\sym{*}" 0.100 "\sym{**}" 0.050 "\sym{***}" 0.010) replace 
