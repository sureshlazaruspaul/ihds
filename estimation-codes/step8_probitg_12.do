//==============================================================================
//	STEP 8 : PROBIT REGRESSIONS -WEIGHTED (2012); SEX = FEMALE 
//==============================================================================
//  Instructions:
//------------------------------------------------------------------------------
...
// open logfile ... 
//------------------------------------------------------------------------------
log using "C:\ihds\step8_probitg_12", replace
...
foreach m of local depvars { 
	...
	pe tab sample, missing 
        ...
        //--------------------------------------------------------------------------
        local k  `x' stateid groups8x female 
	...
	* adolescents rural ed4x
	...
	pe svyset psuid [pw = weight] , singleunit(scaled) strata(newstrat) 
        ...
        pe svy, subpop(sample) : probit `m' i.fu1 `cvars' `ovars' 

		pe eststo: margins , dydx(fu1) subpop(if sample==1) vce(unconditional) // marginal effects 

} 


// output coefficients to Latex
esttab using step8_probitg_12.tex , b(a6) se(6) wide starlevels("\sym{*}" 0.100 "\sym{**}" 0.050 "\sym{***}" 0.010) replace 
