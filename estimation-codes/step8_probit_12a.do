...
// open logfile ... 
//------------------------------------------------------------------------------
log using "C:\ihds\step8_probit_12a", replace
...
foreach m of local depvars { 
	...
	* adolescents rural ed4x
	...
	pe svyset psuid [pw = weight] , singleunit(scaled) strata(newstrat) 
        ...
        pe svy, subpop(sample) : probit `m' `elecvar' `cvars' `ovars' 
...
esttab using step8_probit_12a.tex , b(a6) se(6) wide starlevels("\sym{*}" 0.100 "\sym{**}" 0.050 "\sym{***}" 0.010) replace 
