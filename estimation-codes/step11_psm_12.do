//==============================================================================
//	STEP 11 : PROPENSITY SCORE MATCHING REGRESSIONS (2012) 
//==============================================================================
//  Instructions:
//------------------------------------------------------------------------------
...
// open logfile ... 
//------------------------------------------------------------------------------
log using "C:\ihds\step11_psm_12", replace
...
foreach m of local depvars { 
	...
	pe keep if inlist( sample, 1 ) // reg sample 
        ...
        di _n "-----------------------------------------------------------------" 
		di    "{title:PROPENSITY SCORE MATCHING REGRESSIONS}"
        di    "-----------------------------------------------------------------" 
...
esttab using step11_psm_12.tex , b(a6) se(6) wide starlevels("\sym{*}" 0.100 "\sym{**}" 0.050 "\sym{***}" 0.010) replace 
...
foreach m of local depvars { // loop for each dependent variable 


pe use elecdat , clear 
	...
	pe psmatch2 fu1 `cvars' `ovars' , kernel kerneltype(biweight) outcome(`m') bwidth( 0.1 ) common trim( 10 ) 
		pe pstest `cvars' `ovars' , both rubin 

	pe glm `m' fu1 `cvars' `ovars' if _support == 1 , fam(bin) link(probit) vce( robust )  

	// marginal effects
	pe eststo: margins , dydx(*) post 
